...
uint8_t sNum[5];
uint8_t opened = 0; // opened = 0 (chiuso) opened = 1 (aperto)
  ...
  while (1)
  {
	  ...
	  if(!MFRC522_Request(PICC_REQIDL, str)){
			if(!MFRC522_Anticoll(str)){
				...
				printf("errore anticoll \r \n");
			}
	  	  }
	  	  else{
				printf("errore request \r \n");
	  	  }
...
void avvia_antifurto(){
	...
	while(button_flag == 0){
		__HAL_TIM_SET_COMPARE(&htim15, TIM_CHANNEL_2, 500);
		HAL_Delay(100);
